...
def handle_dialog(req, res):
    try:
        ...
        if req['session']['new']:
            ...
            sessionStorage[user_id] = {"auth": 0, "key": -1}
        ...
        if req['request']['original_utterance'].lower() == "авторизовать" and sessionStorage[user_id]["auth"] == 0:
            ...
            if int(req['request']['original_utterance'].lower()) in stats.keys():
                ...
                res['response']['text'] = "Авторизация успешна! Для получения статистики," \
                                          " введите одно из слов: Шаги, Активность, Сердце, Бег"
                sessionStorage[user_id]["auth"] = 1
                return
            else:
        ...
        if sessionStorage[user_id]["auth"] == 1:
            ...
            if req['request']['original_utterance'].lower() == "шаги":
                res['response']['text'] = "Ваша статистика за 6, 12, 24, 48, 72 144 часа : " \
                                          + str(st["steps"][0]) + ", " + str(st["steps"][1]) \
                                          + ", " + str(st["steps"][2]) + ", " + str(st["steps"][3]) \
                                          + ", " + str(st["steps"][4]) + ", " + str(st["steps"][5])
                return
            elif req['request']['original_utterance'].lower() == "активность":
                res['response']['text'] = "Ваша статистика за 6, 12, 24, 48, 72 144 часа : " \
                                          + str(st["activity"][0]) + ", " + str(st["activity"][1])\
                                          + ", " + str(st["activity"][2]) + ", " + str(st["activity"][3]) \
                                          + ", " + str(st["activity"][4]) + ", " + str(st["activity"][5])
                return
            elif req['request']['original_utterance'].lower() == "сердце":
                res['response']['text'] = "Ваша статистика за 6, 12, 24, 48, 72 144 часа : " \
                                          + str(st["health"][0]) + ", " + str(st["health"][1])\
                                          + ", " + str(st["health"][2]) + ", " + str(st["health"][3])\
                                          + ", " + str(st["health"][4]) + ", " + str(st["health"][5])
                return
            elif req['request']['original_utterance'].lower() == "бег":
                res['response']['text'] = "Ваша статистика за 6, 12, 24, 48, 72 144 часа : " \
                                          + str(st["running"][0]) + ", " + str(st["running"][1]) \
                                          + ", " + str(st["running"][2]) + ", " + str(st["running"][3])\
                                          + ", " + str(st["running"][4]) + ", " + str(st["running"][5])
                return
